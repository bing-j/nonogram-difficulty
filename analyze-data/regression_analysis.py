...
- analyze-data/out_features/behavioral_features.csv  (optional)
    Provides skill_nonogram for per-experience-group stratification.
    Generated by extract_behavioral_features.py. If absent, per-group
    analysis is skipped gracefully.

Outputs
-------
- analyze-data/out_features/figures/spearman_rank_scatter.png
    3×2 grid of BT rank vs SAT metric rank (all participants).
- analyze-data/out_features/figures/regression_per_puzzle_means.png
    Per-puzzle mean difficulty vs SAT predictor, with error bars (all participants).
- analyze-data/out_features/figures/spearman_rank_scatter_{group}.png
    Per-experience-group scatter grids (beginner / intermediate / experienced).
- analyze-data/out_features/figures/regression_per_puzzle_means_{group}.png
    Per-experience-group mean difficulty plots.
...
EXPERIENCE_BINS: list[int] = [0, 3, 6, 10]   # right-inclusive: (0,3], (3,6], (6,10]
EXPERIENCE_LABELS: list[str] = ["beginner", "intermediate", "experienced"]
...
def load_skill_nonogram(features_csv: Path) -> "pd.DataFrame | None":
    """Load participant skill levels from behavioral_features.csv.

    Returns a DataFrame with columns participant_id, skill_nonogram, or None
    if the file is not found (per-group analysis will be skipped).
    """
    if not features_csv.exists():
        print(
            f"  [WARNING] Features CSV not found: {features_csv}\n"
            "  Per-group analysis will be skipped. Run extract_behavioral_features.py first.",
            file=sys.stderr,
        )
        return None
    df = pd.read_csv(features_csv, usecols=["participant_id", "skill_nonogram"])
    df["skill_nonogram"] = pd.to_numeric(df["skill_nonogram"], errors="coerce")
    return df.drop_duplicates("participant_id")[["participant_id", "skill_nonogram"]]
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(
        description="SAT vs. human difficulty: Spearman rank correlation analysis."
    )
    ap.add_argument(
        "--features_csv",
        type=Path,
        default=REPO_ROOT / "analyze-data" / "out_features" / "behavioral_features.csv",
        help="Path to behavioral_features.csv (used for experience group stratification).",
    )
    args = ap.parse_args()
    ...
    # Optionally join skill_nonogram for per-group stratification
    skill_df = load_skill_nonogram(args.features_csv)
    if skill_df is not None:
        ratings_df = ratings_df.merge(skill_df, on="participant_id", how="left")
    ...
    if "skill_nonogram" not in ratings_df.columns:
        print(
            "\n[WARNING] 'skill_nonogram' not available — skipping per-group analysis.\n"
            "  Run extract_behavioral_features.py and retry to enable group stratification."
        )
    else:
        df_grouped = assign_groups(ratings_df)
        print("\n" + "=" * 70)
        print("EXPERIENCE GROUP BREAKDOWN")
        print("=" * 70)
        for label in EXPERIENCE_LABELS:
            group_r = df_grouped[df_grouped["experience_group"] == label]
            n_participants = group_r["participant_id"].nunique()
            print(f"  {label:<14}: {n_participants:>2} participants, {len(group_r):>3} rows")

        for label in EXPERIENCE_LABELS:
            group_r = df_grouped[df_grouped["experience_group"] == label].copy()
            n_participants = group_r["participant_id"].nunique()
            ...
            print(f"\n{'=' * 70}")
            print(f"GROUP: {label.upper()}  ({n_participants} participants)")
            print(f"{'=' * 70}")

            bt_dfs_g = compute_bt_ranks(group_r)
            spearman_results_g = spearman_rank_correlations(bt_dfs_g, sat_df)
            print_spearman_results(spearman_results_g)

            df_merged_g = merge_data(group_r, sat_df)
            plot_rank_scatter(
                bt_dfs_g, sat_df, spearman_results_g,
                FIGURE_DIR / f"spearman_rank_scatter_{label}.png",
            )
            plot_per_puzzle_means(
                df_merged_g,
                FIGURE_DIR / f"regression_per_puzzle_means_{label}.png",
            )
